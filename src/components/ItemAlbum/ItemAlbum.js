...
function ItemAlbum({ data }) {
    ...
    const handleOnClick = () => {
        if (data.allowAudioAds) {
            dispatch(zingCounter.actions.setModalVip(true));
        }
    };
    return (
        <div className={cx('item') + ' l-4 m-6 c-12 m-pro-6'} onDoubleClick={handleOnClick}>
            <div className={cx('include')}>
                ...
                <Button content="Khác" className={cx('more')} iconLeft={<i class="icon ic-more"></i>} />
            </div>
        </div>
    );
}
